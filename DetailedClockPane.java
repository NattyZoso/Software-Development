...
*  Mark Newbury
*  CIS 315
*  JAVA_HA14.0 - DetailedClockPaneClass
*/
...
public class DetailedClockPane extends Pane {
	...
	private void paintClock() {
		 ...
		 * Text t2 = new Text(centerX - clockRadius + 3, centerY + 5, "9"); Text
		 * t3 = new Text(centerX + clockRadius - 10, centerY + 3, "3"); Text t4
		 * = new Text(centerX - 3, centerY + clockRadius - 3, "6");
		 */
		...
		for (int i = 1; i <= 12; i++) {
			...
			getChildren().addAll(new Text(tx + clockRadius + 20, ty + clockRadius + 30, numberStr));
		}

		// Drawing the tick marks for seconds
		...
		for (int i = 1; i <= 60; i++) {
			...
			float x2 = (float) (centerX + Math.cos(angleFrom3) * (clockRadius - length));
			float y2 = (float) (centerY - Math.sin(angleFrom3) * (clockRadius - length));

			Line tickLine = new Line(x1, y1, x2, y2);
